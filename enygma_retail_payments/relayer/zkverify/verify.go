...
// against the chain (an already-spent nullifier, a root that's since rolled
// off the vault's history, etc). That's what the relayer's eth_call dry-run
// is for, and the contract's own on-chain verifier remains the sole source
// of truth — this package exists purely to reject garbage before it ever
// reaches the network, at effectively zero cost (no RPC round-trip,
// milliseconds of local pairing arithmetic).
package zkverify
...
// gnark_circuits/scripts/keys/ holds and the gnark server loads to prove.
// Verifying keys are public; this is not a secret. nbPublic is the exact
// public-signal count the circuit declares (e.g. 9 for
// PaymentRelayerFeePublic); Verify fails closed if the supplied signal
...
// gnark_circuits are separate Go modules deployed as separate services, and
// pulling gnark_circuits in as a relayer dependency would couple the two
...
func Load(vkPath string, nbPublic int) (*Verifier, error) {
	...
	// RELAYER_VK_RELAYER_FEE_PATH pointing at a stale or wrong-circuit key)
	// would surface later as every proof being rejected with the same
	// generic "invalid zk proof" 400, indistinguishable from a genuinely
	// forged proof, instead of a clear configuration error right now.
