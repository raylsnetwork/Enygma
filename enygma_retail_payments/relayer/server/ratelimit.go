...
// not IP, so the moment callers get distinct per-client API keys this
// becomes genuinely per-caller with no further change here. Keying on
// identity rather than IP also survives callers behind shared NAT/proxies
// without either starving each other or sharing one attacker's headroom.
