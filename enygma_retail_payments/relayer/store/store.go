...
type Record struct {
	...
	// ChannelIdx is set only for POST /relay/channel records — the index
	// TagChannelRegistry.openChannel() assigned, parsed from its
	// ChannelOpened event. Zero for every other endpoint's records. Carried
	// here (rather than a generic payload field) so a cached replay of an
	// already-mined channel setup can still return the caller the
	// channelIdx they need to use it — without this, a replayed response
	// would silently come back as 0.
	ChannelIdx uint64 `json:"channelIdx,omitempty"`
}
...
// MarkMinedChannel is MarkMined plus the ChannelOpened event's assigned
// channelIdx — used only by POST /relay/channel, the one endpoint whose
// success response carries more than TxHash/BlockNumber/GasUsed.
func (s *Store) MarkMinedChannel(key, txHash string, blockNumber, gasUsed, channelIdx uint64) error {
	rec := Record{
		Status:      StatusMined,
		TxHash:      txHash,
		BlockNumber: blockNumber,
