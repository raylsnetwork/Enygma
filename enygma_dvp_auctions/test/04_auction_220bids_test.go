...
// End-to-end integration test: 220 bids across 3 batches.
//
// Scenario:
//   Bob locks an ERC-721 NFT (tokenId=42, floorPrice=50).
//   220 bidders each submit a USDC bid:
//     - 219 bidders bid 100 USDC
//     - Bidder at index 55 bids 300 USDC (the clear winner)
//   Auctioneer runs 3 Phase-1 batches (batch capacity = 100):
//     batch 1: bids  0..99   → winner = bidder 55 (amount 300)
//     batch 2: bids 100..199 → winner = bidder 100 (amount 100, first in batch)
//     batch 3: bids 200..219 → winner = bidder 200 (amount 100, first in batch)
//   Auctioneer runs Phase-2 final → overall winner = bidder 55 (300 > 100).
//   Assertions: SETTLED, overallWinnerCommit == bidder55.commitA, bidCount == 220.
...
const (
	numBids220    = 220
	winnerIdx220  = 55  // bidder 55 bids 300; all others bid 100
	winnerAmt220  = 300
	loserAmt220   = 100
)

func TestAuction_OnChain_220Bids(t *testing.T) {
	...
	t.Logf("Generating %d bidder key pairs...", numBids220)
	bidders := make([]*core.SpendKeyPair, numBids220)
	for i := range bidders {
	...
	// ─── USDC commitments for all 220 bidders ────────────────────────────────

	saltsIn     := make([]*big.Int, numBids220)
	amounts     := make([]*big.Int, numBids220)
	usdcCommits := make([]*big.Int, numBids220)

	for i := 0; i < numBids220; i++ {
		saltsIn[i], err = core.RandomInField()
		checkErr(t, fmt.Sprintf("RandomInField(saltIn %d)", i), err)

		amounts[i] = big.NewInt(loserAmt220)
		if i == winnerIdx220 {
			amounts[i] = big.NewInt(winnerAmt220)
		}
	...
	// ─── Seed all 220 USDC notes ──────────────────────────────────────────────

	t.Logf("Seeding %d USDC notes into UsdcVault...", numBids220)
	for i := 0; i < numBids220; i++ {
		...
		if (i+1)%50 == 0 {
			t.Logf("  seeded %d/%d", i+1, numBids220)
		}
	}
	...
	usdcProofs := make([]*core.MerkleProof, numBids220)
	for i := 0; i < numBids220; i++ {
	...
	t.Logf("── Phase 0b: Submitting %d bids ─────────────────────────────────────────", numBids220)
	...
	bidsData := make([]bidRecord, numBids220)
	...
	for i := 0; i < numBids220; i++ {
		...
		if (i+1)%50 == 0 {
			t.Logf("  submitted bid %d/%d", i+1, numBids220)
		}
	}
	t.Logf("All %d bids submitted", numBids220)
	...
	// ─── Phase 1: 3 batch proofs ──────────────────────────────────────────────

	numBatches := (numBids220 + core.AuctionBatchSize - 1) / core.AuctionBatchSize // = 3
	...
	for b := 0; b < numBatches; b++ {
		...
		// Remaining slots are zero (inactive) — already zero-valued by default.
	...
	// Winner is bidder 55 (batch 1). Their saltB is bidsData[winnerIdx220].saltB.
	...
	finalResult, err := gnarkClient.AuctionFinalProof(core.AuctionFinalParams{
		...
		BobPk:       bob.PublicKey,
		SaltB:       bidsData[winnerIdx220].saltB,
	...
	if onChainWinner.Cmp(bidsData[winnerIdx220].commitA) != 0 {
		t.Errorf("overallWinnerCommit mismatch: got %s, want bidder55's commitA %s",
			onChainWinner, bidsData[winnerIdx220].commitA)
	} else {
		t.Logf("✓ overallWinnerCommit == bidder %d's commitA (highest bid: %d USDC)", winnerIdx220, winnerAmt220)
	}

	if onChainBidCount.Cmp(big.NewInt(numBids220)) != 0 {
		t.Errorf("bidCount mismatch: got %s, want %d", onChainBidCount, numBids220)
	} else {
		t.Logf("✓ bidCount == %d", numBids220)
	}

	if winningAmount.Cmp(big.NewInt(winnerAmt220)) != 0 {
		t.Errorf("winningAmount mismatch: got %s, want %d", winningAmount, winnerAmt220)
	} else {
		t.Logf("✓ winningAmount == %d USDC", winnerAmt220)
	}

	t.Logf("✓ 220-bid on-chain auction test complete")
}
