...
describe("Auditability Test", () => {
    ...
    it(`Admin should register erc1155 tokens properly. `, async () => {
        ...
        NFT_ID = jsUtils.buffer2BigInt(Buffer.from(crypto.randomBytes(1))) % jsUtils.SNARK_SCALAR_FIELD;
        FT_ID = jsUtils.buffer2BigInt(Buffer.from(crypto.randomBytes(1))) % jsUtils.SNARK_SCALAR_FIELD;
    ...
    it("Admin mints non-fungible Erc1155 for Alice and fungible Erc1155 for Bob.", async () => {
        ...
        bobDepositAmount = jsUtils.buffer2BigInt(Buffer.from(crypto.randomBytes(1)));
        await owner.calls.erc1155.mint(
            bob.wallet.address, 
            FT_ID, 
            bobDepositAmount * 2n , 
    ...
    it("Alice submits her proof through a Relayer.", async () => {
        ...
        const uniqueId = jsUtils.erc1155UniqueId(alice.knows.payment.tokenAddress, alice.knows.payment.tokenId, alice.knows.delivery.price);
        ...
        for(var i = 0; i < deliveryProof.numberOfOutputs; i++){
            tempStorage.push(deliveryProof.statement[1 + i + 3 * deliveryProof.numberOfInputs])
        }
    ...
    it("Bob submits his proof through a Relayer. ", async () => {
        ...
        const uniqueId = jsUtils.erc1155UniqueId(bob.knows.delivery.tokenAddress, bob.knows.delivery.tokenId, 1n);
        ...
        const paymentProof = await prover.prove(
            "JoinSplitErc1155WithAuditor",
            {
                ...
                treeNumbers:
                            [
                                bob.coins[0].treeNumber, 
                                bob.coins[1].treeNumber
                            ],
                merkleProofs:
        ...
        console.log("[NOT-IMPLEMENTED] Auditor has access to calldata history or receiving the events ")
        auditor.knows.bobStatments = paymentProof.statement;
    });

    it("Auditor opens the Alice's transaction's private information.", async () => {
        
        console.log("Decrypting Alice's statements' encryptedData with Auditor's privateKey");
        ...
        const decrypted = jsUtils.poseidonDecryptWrapper(babyJub, encrypted, authKey, nonce, auditor.keyPair.privateKey, length);
    ...
    it("Auditor opens the Bob's transaction's private information.", async () => {
        ...
        console.log("Decrypting Bobs statements' encryptedData with Auditor's privateKey");
