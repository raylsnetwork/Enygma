...
import {IGenericGroth16Verifier} from "../interfaces/IGenericGroth16Verifier.sol";
...
contract Verifier is IVerifier, AccessControl {
    ...
    IZkDvp.VerifyingKey[MAX_NUMBER_OF_CIRCUITS] private _verificationKeys;
    uint256 _vkLength = 0;
    ...
    function initializeVerifier(address groth16Verifier_) public returns (bool) {
        _groth16Verifier = groth16Verifier_;
        _setupRole(DEFAULT_OWNER_ROLE, msg.sender);
        return true;
    }

    function addVerificationKey(IZkDvp.VerifyingKey memory vKey_) public onlyRole(DEFAULT_OWNER_ROLE) returns(bool) {
        ...
        for (uint8 j = 0; j < vKey_.ic.length; j++) {
            // IC
            _verificationKeys[i].ic.push(IZkDvp.G1Point(vKey_.ic[j].x, vKey_.ic[j].y));
        }
    ...
    function verifyProof(
            ...
            IZkDvp.SnarkProof memory proof_, 
            uint256[] memory inputs_
    )public view returns(bool){
        return IGenericGroth16Verifier(_groth16Verifier)
            .verify(
