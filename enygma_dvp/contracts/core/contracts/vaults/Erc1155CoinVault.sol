...
contract Erc1155CoinVault is AbstractCoinVault, ERC1155Holder {
    ...
    uint256 constant public VK_ID_ERC1155_1 = 2; // nonfungible
    uint256 constant public VK_ID_ERC1155_FUNG_1 = 3;
    uint256 constant public VK_ID_ERC1155_2 = 4; // fungible joinSplit
    uint256 constant public VK_ID_ERC1155_10 = 5; // non-fungible batch
    uint256 constant public VK_ID_ERC1155_FUNG_BROKER = 14;
    uint256 constant public VK_ID_ERC1155_FUNG_AUDITOR = 15;
    uint256 constant public VK_ID_ERC1155_NON_FUNG_AUDITOR = 16;
    ///////////////////////////////////////////////
    //              Constructor
    //////////////////////////////////////////////
    ...
    function deposit(
        ...
        if(params.length == 3){
            ...
            for (uint i = 0; i < tokenIds.length; i++) {
                ...
                if(IRaylsERC1155(_assetContractAddress).isFungible(tokenIds[i])){
                    ...
                    uint256 uid1 = IPoseidonWrapper(_hashContractAddress).poseidon(
                        [uint256(uint160(_assetContractAddress)), tokenIds[i]]
                    );
...
//     commitments[numberOfOutputs];
    ...
    function checkReceiptConditions(
        ...
        uint256 expectedNormalSize = 3 + 3 * receipt.numberOfInputs + receipt.numberOfOutputs;
        // uint256 expectedBrokerSize = 5 + 3 * receipt.numberOfInputs + receipt.numberOfOutputs;
        // size broker-enabled statement: 3 + 3 * numberOfInputs + numberOfOutputs + 4

        uint256 expectedFungibleAuditorSize = expectedNormalSize + 5 + getEncryptionSizeForFungible(receipt.numberOfInputs, receipt.numberOfOutputs);
        uint256 expectedNonFungibleAuditorSize = expectedNormalSize + 5 + getEncryptionSizeForNonFungible(receipt.numberOfInputs);
        ...
        if(receipt.statement.length == expectedNormalSize){
            ...
            if(receipt.numberOfInputs != 1 && receipt.numberOfOutputs != 2 && receipt.numberOfOutputs != 10){
        ...
        if(receiptType == 5){
            ...
            if(jInputSize == 2){ // JoinSplit broker-enabled
                IVerifier(_verifierContractAddress).verifyProof(
    ...
    function verifyAuditorPublicKey(IZkDvp.ProofReceipt memory receipt) internal view returns(bool){
        uint256[2] memory auditorPublicKey;

        uint256 pkIndex = receipt.numberOfInputs * 3 + receipt.numberOfOutputs + 1;
        auditorPublicKey[0] = receipt.statement[pkIndex];
        auditorPublicKey[1] = receipt.statement[pkIndex + 1];

        if(!(IZkDvp(_zkDvpContractAddress).isAuditorRegistered(auditorPublicKey)))
        {
