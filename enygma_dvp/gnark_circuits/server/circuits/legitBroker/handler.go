...
func NewHandler(pkPath, vkPath string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
        var request LegitBrokerRequest
		...
		witness:=templates.LegitBrokerCircuit{
		
		}
	

		witness.StBeacon = frontend.Variable(request.StBeacon)
		witness.StBlindedPublicKey = frontend.Variable(request.StBlindedPublicKey)
		...
		publicSignal =  append(publicSignal, utils.ParseBigInt(request.StBlindedPublicKey))
		
		c.JSON(http.StatusOK, LegitBrokerOutput{
            Proof:  proofRemix,
