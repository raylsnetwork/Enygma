...
func NewHandler(pkPath, vkPath string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
        var request JoinSplitERC20_10_2Request
		...
		joinsplit_erc20_10_2_config := templates.Erc20CircuitConfig{
			TmNInputs: 10,
			TmMOutputs: 2,
			TmMerkleTreeDepth:8,
		...
		for i := range witness.WtPathElements {
			...
			witness.WtPathElements[i] = make([]frontend.Variable, joinsplit_erc20_10_2_config.TmMerkleTreeDepth)
		}
		...
		ccs, err := frontend.Compile(ecc.BN254.ScalarField(), r1cs.NewBuilder, &circuitJoinSplitERC20_10_2)
		...
		c.JSON(http.StatusOK, JoinSplitERC20_10_2Output{
            Proof:  proofRemix,
