...
func NewHandler(pkPath, vkPath string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
        var request AuctionPrivateOpeningRequest
		...
		circuitPrivateOpening :=templates.AuctionPrivateOpeningCircuit{
			
		}

		witness:=templates.AuctionPrivateOpeningCircuit{
		...
		witness.Config = privateOpeningConfig

		solver.RegisterHint(primitives.ModHint)
		solver.RegisterHint(primitives.ERC155UniqueIdNative)
		solver.RegisterHint(primitives.PoseidonNative)

		ccs, err := frontend.Compile(ecc.BN254.ScalarField(), r1cs.NewBuilder, &circuitPrivateOpening)
		...
		publicSignal =  append(publicSignal, utils.ParseBigInt(request.WtBidAmount))
		publicSignal =  append(publicSignal, utils.ParseBigInt(request.WtBidRandom))
	

		c.JSON(http.StatusOK, AuctionPrivateOpeningROutput{
            Proof:  proofRemix,
