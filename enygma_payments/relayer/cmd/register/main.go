...
func main() {
	...
	// Fix H-02 residual: registerAccount now takes a pre-computed
	// commitment point (initialCommitX, initialCommitY) instead of a raw
	// randomness scalar — compute Com(0, randomness) the same way every
	// other account does, via the contract's own pedCom (a view call, no
	// tx), rather than reimplementing BabyJubJub curve arithmetic here.
	// (publicKey/randomness are real, not dummy values, per this file's
	// own top-of-file doc comment: the USDr fee-proof feature requires
	// the relayer to be a genuine k=6 anonymity-set participant now.)
	initialCommitX, initialCommitY, err := instance.PedCom(&bind.CallOpts{}, big.NewInt(0), randomness)
	if err != nil {
		log.Fatalf("pedCom(0, randomness): %v", err)
	}
	tx, err := instance.RegisterAccount(auth, relayerAddr, big.NewInt(*accountID), publicKey, initialCommitX, initialCommitY, []byte{})
	if err != nil {
